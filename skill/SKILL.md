...
**After editing any script in this skill, run `./scripts/tests/run-all.sh`.** No stack needed — it tests the scripts, not a migration. Three suites: the panel parser against a fixture exercising every shape it claims to handle; `plan-collector.py` asserted against one field per verdict (its output is a *classification*, where a wrong answer looks exactly like a right one); and a check that `receiver-map.json` and `references/integration-to-receiver.md` still agree, since the same mapping is deliberately written twice and nothing else notices when they diverge.
...
| `scripts/ecs-to-clickhouse.py` | the customer is keeping Beats or Elastic Agent — lands ECS documents with their own schema, one dense view per dataset, and prints the sources to register |
| `references/sources.md` | mapping data views to sources; ad-hoc views, runtime fields, cross-cluster — and whether the target can keep the SOURCE's schema. **Read it before any proof of concept that keeps Filebeat/Metricbeat or Elastic Agent**: ECS can stay, on a `log` source, but a wide multi-metricset table silently dilutes every average |
...
| `--triage` | **Measured** on the same estate (37 dashboards, 347 panels): **128 ready / 216 decide / 3 blocked** — re-confirmed 2026-09-18. Quote it by estate, not blended, because two thirds of those panels were never migrated: the five integrations are **137 panels, 93 with no direct translation (68%)**; kubernetes is 130 / 96 (74%); Windows Security is 80 / 27 (34%). And read `decide` as the classifier defines it — `ready` means *a direct translation exists*, so its complement is "no direct builder path", which includes panels needing a hand-written `sql` tile, not only panels needing a human judgement call. The heatmap rule was added 2026-09-17 after ClickStack's heatmap turned out to have no categorical axis — it had been calling two unmigratable panels "ready". It was 140/189/3 until the mysql migration showed the metric rules were too lenient — a panel doing `average()`/`max()` on a metric's *value* has no builder path whether the field is a Sum or a Gauge, and a `table` tile takes no row limit, so `terms(f) size=N` on a datatable needs SQL. 22 panels moved from ready to decide, which is the more honest split |
...
| Keeping the SOURCE's schema on the target | **Measured 2026-09-18**, and it corrected this skill's own claim that metrics could not be kept. ECS as flat typed columns: a **log** source maps it via expressions, and that includes **wide ECS metrics** — 1,005 series over 36,000 real Metricbeat documents matched Elasticsearch across the whole tile vocabulary. A **metric** source still cannot, because it requires the narrow OTel metric tables. The load-bearing finding is the trap, not the capability: a numeric builder aggregation compiles to `AVG(toFloat64OrDefault(toString(col)))`, so on a sparse multi-metricset table a NULL is **a zero that counts** — `avg`, `min`, `last_value`, `count` and `quantile` are silently wrong while `max`, `sum` and `count_distinct` are right, and all of them render. `verify/verify-ecs-source.py` asserts all of it in 31 checks with 6 mutations. **The coercion is not ECS-only**: a missing Map key reads as `''` and casts to zero too, so `avg(LogAttributes['x'])` on a plain OTel logs source is diluted whenever some rows lack the key — measured 14% low on this repo's own data. `references/sources.md` |
