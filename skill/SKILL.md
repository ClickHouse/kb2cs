...
| Keeping the SOURCE's schema on the target | **Measured 2026-09-18**, and it corrected this skill's own claim that metrics could not be kept. ECS as flat typed columns: a **log** source maps it via expressions, and that includes **wide ECS metrics** — 1,005 series over 36,000 real Metricbeat documents matched Elasticsearch across the whole tile vocabulary. A **metric** source still cannot, because it requires the narrow OTel metric tables. The load-bearing finding is the trap, not the capability: a numeric builder aggregation compiles to `AVG(toFloat64OrDefault(toString(col)))`, so on a sparse multi-metricset table a NULL is **a zero that counts** — `avg`, `min`, `last_value`, `count` and `quantile` are silently wrong while `max`, `sum` and `count_distinct` are right, and all of them render. `verify/verify-ecs-source.py` asserts all of it in 29 checks with 6 mutations. `references/sources.md` |
